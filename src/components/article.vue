...
<div class="content">
  <div class="cont">
    <h3>{{json.title}}</h3>
    <div class="time"><p>{{json.time|date}} <span><img src="../assets/img/zan.png" alt=""/></span></p></div>
  ...
  export default{
    ...
    mounted(){
      this.$http.get('/src/data/article.data').then((res)=>{
        console.log(res.data[this.$route.params.aid])
       this.json=res.data[this.$route.params.aid-1]
      },(error)=>{
        alert('article.vue'+error.status)
      })
    }
  }
